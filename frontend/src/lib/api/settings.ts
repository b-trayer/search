...
export async function applyPreset(preset: WeightPreset | string): Promise<RankingWeights> {
  const response = await safeFetch(
    `${API_BASE}/api/v1/settings/presets/${encodeURIComponent(preset)}`,
    { method: 'POST' },
  );
  return handleResponse<RankingWeights>(response);
}
...
export async function saveCustomPreset(
  ...
  const response = await safeFetch(`${API_BASE}/api/v1/settings/custom-presets`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteCustomPreset(name: string): Promise<void> {
  const response = await safeFetch(
    `${API_BASE}/api/v1/settings/custom-presets/${encodeURIComponent(name)}`,
    { method: 'DELETE' },
  );
