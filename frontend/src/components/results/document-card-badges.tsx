      ...
      {subjects.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {subjects.slice(0, 4).map((subject, idx) => {
            ...
            const display = subject.length > 50 ? subject.slice(0, 50) + '…' : subject;
            return (
              <Badge
                key={idx}
                variant="outline"
                title={subject.length > 50 ? subject : undefined}
                className={`text-xs ${
                  isMatching
          ...
          {subjects.length > 4 && (
            <Badge
              variant="outline"
              title={subjects.slice(4).join(', ')}
              className="text-xs text-notion-text-tertiary border-notion-border"
            >
              +{subjects.length - 4}
            </Badge>
          )}
