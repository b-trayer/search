    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        {onResetSection && (
          <SectionResetButton
            ...
            sectionName="Скоры f_topic"
          />
        )}
      </div>
      <p className="mt-1 text-sm text-notion-text-secondary">
        Базовая часть f_topic в зависимости от типа совпадения со специализацией.
        Логично: прямое совпадение ≥ совпадение ключевых слов. Бонус за интересы
        пользователя (b = 0.2 за каждый матч, не более 0.5) добавляется к этой
        базе и не настраивается через интерфейс.
      </p>
        ...
        {Object.entries(TOPIC_SCORE_LABELS).map(([key, label]) => {
          ...
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <label className="text-sm font-medium text-notion-text">{label}</label>
      ...
      {!monotonic && (
        <div className="mt-3 flex items-start gap-2 rounded-notion border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
          ...
          <span>
            Нарушена монотонность: ожидается {direct.toFixed(1)} ≥ {keyword.toFixed(1)}.
            Сейчас более точное совпадение оценивается ниже более общего.
            Это может приводить к нелогичному ранжированию.
          </span>
        </div>
      )}
    </section>
