        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-notion-border">
              <th className="text-left py-2 px-2 text-notion-text-secondary font-medium">Роль \ Тип</th>
              {docTypes.map((dt) => (
                <th key={dt} className="text-center py-2 px-2 text-notion-text-secondary font-medium">
          ...
          <tbody>
            {roles.map((role) => (
              <tr key={role} className="border-b border-notion-border/50">
                <td className="py-2 px-2 font-medium text-notion-text">{ROLE_LABELS[role] || role}</td>
                {docTypes.map((dt) => (
                  <td key={dt} className="py-2 px-2">
                    <input
                      type="number"
                      min={0}
                      max={1}
                      ...
                      className="w-20 h-8 text-center text-sm border border-notion-border rounded-md bg-notion-bg focus:outline-none focus:ring-1 focus:ring-notion-accent"
                      disabled={isSaving}
                    />
                  </td>
                ))}
