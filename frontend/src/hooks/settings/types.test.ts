...
describe('initialSettingsData', () => {
  it('has correct default values', () => {
    ...
    expect(initialSettingsData.presetMap).toEqual({});
    expect(initialSettingsData.customPresets).toEqual([]);
  });
});
...
describe('checkHasChanges', () => {
  const baseData: SettingsData = {
    ...
    presetMap: {},
    customPresets: [],
    roleTypeMatrix: { bachelor: { textbook: 0.5 } },
    originalRoleTypeMatrix: { bachelor: { textbook: 0.5 } },
  ...
  it('returns true when roleTypeMatrix changed', () => {
    const data = {
      ...baseData,
      roleTypeMatrix: { bachelor: { textbook: 0.7 } },
    };
    expect(checkHasChanges(data)).toBe(true);
  });
