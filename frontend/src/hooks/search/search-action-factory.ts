...
export function createSearchAction({
  ...
  return async (
    ...
    dispatch({ type: 'SET_LAST_PARAMS', payload: { userId, enablePersonalization, filters, searchField, sortBy } });
    dispatch({ type: 'SEARCH_START' });

    try {
      const response = await searchDocuments(query, userId, enablePersonalization, page, perPage, filters, searchField, sortBy);
