...
class TestStatsEndpoint:
    ...
    async def test_get_stats_success(self):
        with patch("backend.app.api.interactions.get_total_stats") as mock_stats, \
             patch("backend.app.api.interactions._get_total_documents") as mock_docs:
            mock_stats.return_value = {
                "total_impressions": 500,
                "total_clicks": 50
            }
            mock_docs.return_value = 276152
            ...
            assert data["total_documents"] == 276152

    async def test_get_stats_returns_zero_when_opensearch_fails(self):
        with patch("backend.app.api.interactions.get_total_stats") as mock_stats, \
             patch("backend.app.api.interactions._get_total_documents") as mock_docs:
            mock_stats.return_value = {"total_impressions": 0, "total_clicks": 0}
    ...
    async def test_total_documents_helper_returns_count(self):
        from backend.app.api.interactions import _get_total_documents

        opensearch = AsyncMock()
        opensearch.count.return_value = {"count": 12345}

        result = await _get_total_documents(opensearch)
        assert result == 12345

    async def test_total_documents_helper_swallows_opensearch_errors(self):
        from opensearchpy import OpenSearchException
        from backend.app.api.interactions import _get_total_documents

        opensearch = AsyncMock()
        opensearch.count.side_effect = OpenSearchException("boom")
